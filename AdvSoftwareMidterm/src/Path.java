import javafx.scene.image.Image;

public class Path {
	//This Counter will keep track of how far along the player is in the story
	int counter = 0;

	//This will be the area for generating content for their journey
	public String toString() { 
		return " "; 
	}

	// This is the constructor where the counter is stored.
	Path(int count) {
		this.counter = count;
	}

	Image building1 = new Image("Images/Abandoned Building.png");
	Image forest1 = new Image("Images/CreepyForest1.png");
	Image forest2 = new Image("Images/CreepyForest2.png");
	Image forest3 = new Image("Images/CreepyForest3.png");
	Image forest4 = new Image("Images/CreepyForest4.png");
	Image forest5 = new Image("Images/CreepyForest5.png");
	Image gameOver = new Image("Images/GameOVer.png");
	Image forest6 = new Image("Images/HauntedForest6.png");
	Image maze1 = new Image("Images/HauntedMaze.png");
	Image maze2 = new Image("Images/HauntedMazeTwo.png");
	Image maze3 = new Image("Images/HauntedMazeThree.png");
	Image house1 = new Image("Images/SwampHouse.png");
	Image road1 = new Image("Images/CreepyRoad.png");
	Image beginning = new Image("Images/HauntedMaze.png");
	Image monkey1 = new Image("Images/DesrtMonkey.png");
	Image bananaBasket = new Image("Images/BannanaDefense.png");
	Image bananaLoss = new Image("Images/BasketLoss.png");
	Image snakeLoss = new Image("Images/DanceLoss.png");
	Image tigerLoss1 = new Image("Images/EatenTiger.png");
	Image snake1 = new Image("Images/ForestSnake.png");
	Image hole1 = new Image("Images/Hole.png");
	Image hole2 = new Image("Images/HoleLoss.png");
	Image tiger1 = new Image("Images/Jungle Tiger.png");
	Image steak1 = new Image("Images/NecklaceSteak.png");
	Image roadLoss = new Image("Images/RoadLoss.png");
	Image snake2 = new Image("Images/SnakeDance.png");
	Image snake3 = new Image("Images/SnakeRoad.png");
	Image snake4 = new Image("Images/SnakeTree.png");
	Image steakLoss = new Image("Images/SteakLose.png");
	Image tiger2 = new Image("Images/TigerFight.png");
	Image treeLoss = new Image("Images/TreeLoss.png");
	
	//add images and make sure they upload
	
	
}
